...
export default class Upload {
  ...
  async list () {
    const { uploaded } = await store.get('uploaded')
    const { deleted } = await store.get('deleted')
    return (uploaded || []).filter(obj => !(deleted || []).includes(obj.url) && !(deleted || []).includes(obj.delete)).reverse()
  }

  async _save (obj) {
    const { uploaded } = await store.get('uploaded')
    store.set({ uploaded: [...(uploaded || []), obj] })
    return true
  }

  async _remove (url) {
    const { deleted } = await store.get('deleted')
    store.set({ deleted: [...(deleted || []), url] })
    return true
  }

  async _clear () {
    await store.remove(['uploaded', 'deleted'])
    return true
  }
}
...
export class SmmsUpload extends Upload {
  ...
  async delete (url) {
    ...
    const isDeleted = data.indexOf('File delete success')
    isDeleted && this._remove(url)
    return isDeleted
  }

  async clear () {
    const { data } = await axios.get('https://sm.ms/api/clear')
    const cleared = data.code === 'success'
    cleared && this._clear()
    return cleared
  }
  ...
  //   if (res.code === 'success') {
  //     const { deleted: data } = await store.get('deleted')
  //     const deleted = (data || [])
  //     return (res.data || [])
  //       .filter(item => !deleted.includes(item.delete))
  //       .map(item => ({ url: item.url, delete: item.delete }))
  //       .reverse()
  //   } else {
  //     return []
  //   }
...
export class WeiboUpload extends Upload {
  ...
  _pid2url (params) {
    ...
    // 协议：https://ws http://ww
    // 图片尺寸：thumbnail(缩略图) mw690(中等尺寸) large(原图)
    return `https://ws${zone}.sinaimg.cn/large/${params.pid}${params.ext}`
  }
}
